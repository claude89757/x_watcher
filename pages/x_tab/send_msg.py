...
def send_msg(db: MySQLDatabase):
    ...
    # 获取关键词列表
    keywords = db.get_all_x_message_keywords()
    if not keywords:
    ...
    all_messages = db.get_x_messages(selected_keyword)
    if not all_messages:
        st.warning("没有找到推广消息")
        return
    ...
    if processing_messages:
        ...
        # 获取正在处理消息的worker IP
        worker_ips = set(db.get_worker_ip_for_processing_x_messages(selected_keyword))
        ...
        while not all_completed and time.time() - start_time < 600:  # 最多等待10分钟
            ...
            current_status = db.get_x_messages_status([msg['user_id'] for msg in processing_messages])
    ...
    if account_ids:
        if st.button("开始发送", key="send_msg_button", type="primary"):
            ...
            with st.spinner("正在启动消息发送任务..."):
                if len(account_ids) == 1:
                    ...
                    response = requests.post(
                        f"http://{worker_ip}:5000/send_x_promotion_messages",
                        json={
            ...
            while not all_completed and time.time() - start_time < 600:  # 最多等待10分钟
                ...
                current_status = db.get_x_messages_status([msg['user_id'] for msg in user_messages])
                completed_count = sum(1 for status in current_status if status in ['sent', 'failed'])
