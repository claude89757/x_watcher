...
MESSAGES_CACHE_FILE = 'x_messages_cache.json'
...
def generate_msg(db: MySQLDatabase):
    ...
    # 获取高意向客户数据
    high_intent_customers = db.get_second_round_analyzed_x_comments(selected_keyword)
    ...
    # 可编辑的prompt模板
    default_prompt = """基于以下信息为多个用户生成个性化的X平台私信内容:
...
"""
    ...
    if st.session_state.generated_messages:
        ...
        if st.button("保存推广信息", type="primary"):
            ...
            for user_id, message in updated_messages.items():
                db.save_x_message(selected_keyword, user_id, message)
